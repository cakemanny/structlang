...
#define elprintf(fmt, fn, lineno, ...) do { \
    ...
    ast_printf(stderr, "%s:%d: %serror:%s " fmt, fn, lineno, \
...
{
    sl_decl_t* d;
    for (d = sem_info->si_program; d; d = d->dl_list) {
        if (d->dl_name == fn_name) {
            return d;
        }
    }
    return NULL;
        ...
        case '/':
            ...
            if (ty_cmp(t1, int_type) != 0) {
                elprintf("operands of binary operation '%c' should be of "
                        "type 'int'\n", info->si_filename, expr->ex_left->ex_line,
                        expr->ex_op);
                result -= 1;
            }
            if (ty_cmp(t2, int_type) != 0) {
                elprintf("operands of binary operation '%c' should be of "
                        "type 'int'\n", info->si_filename, expr->ex_right->ex_line,
                        expr->ex_op);
                result -= 1;
            }
...
static int verify_expr_let(sem_info_t* info, sl_expr_t* expr)
{
    ...
    // check type exists
    if (!lookup_type_decl(info, expr->ex_type_ann)) {
        elprintf("unknown type: '%T'\n", info->si_filename, expr->ex_line,
                expr->ex_type_ann);
        result -= 1;
    }
    ...
    if (declare_in_current_scope(info, expr->ex_name, expr->ex_type_ann) < 0) {
        elprintf("name '%s' already defined in this scope\n", info->si_filename,
...
static int verify_expr_call(sem_info_t* info, sl_expr_t* expr)
{
    ...
    if (fn_decl == NULL) {
        elprintf("call to undefined function '%s'\n",
                info->si_filename, expr->ex_line, expr->ex_fn_name);
        expr->ex_type = info->si_builtin_types.void_type;
        return result - 1;
    }
    if (fn_decl->dl_tag == SL_DECL_STRUCT) {
        elprintf("struct type '%s' cannot be called like a function\n",
                info->si_filename, expr->ex_line, expr->ex_fn_name);
        // or should this pretend to try be the struct type?
        expr->ex_type = info->si_builtin_types.void_type;
        return result - 1;
    }
    assert(fn_decl->dl_tag == SL_DECL_FUNC);
    ...
    if (num_args < num_params) {
        elprintf("not enough arguments passed to function '%s', expected %d\n",
                info->si_filename, expr->ex_line, expr->ex_fn_name, num_params);
        result -= 1;
...
static int verify_expr_new(sem_info_t* info, sl_expr_t* expr)
{
    ...
    expr->ex_type = ty_type_pointer(ty_type_name(expr->ex_new_ctor));
    return 0;
}
...
static int verify_expr_var(sem_info_t* info, sl_expr_t* expr)
{
    ...
    if (var_type == NULL) {
        elprintf("use of undeclared variable '%s'\n",
                info->si_filename, expr->ex_line, expr->ex_var);
        // maybe type as void?
        return -1;
    }
...
static int verify_expr_return(sem_info_t* info, sl_expr_t* expr)
{
    ...
    if (expr->ex_ret_arg) {
        ...
        if (ty_cmp(expr->ex_ret_arg->ex_type, info->si_current_fn->dl_type) != 0) {
            elprintf("return value with incorrect type '%T', function '%s' "
                    "has return type '%T'\n",
                    info->si_filename, expr->ex_ret_arg->ex_line,
                    expr->ex_ret_arg->ex_type, info->si_current_fn->dl_name,
                    info->si_current_fn->dl_type);
            result -= 1;
        }
    } else {
        VAR(void_type, info->si_builtin_types.void_type);
        if (ty_cmp(void_type, info->si_current_fn->dl_type) != 0) {
...
static int verify_expr_break(sem_info_t* info, sl_expr_t* expr)
{
    ...
    if (info->si_loop_depth == 0) {
        elprintf("occurence of break outside a loop\n",
                info->si_filename, expr->ex_line);
        result -= 1;
    }
...
static int verify_decl_func(sem_info_t* sem_info, sl_decl_t* decl)
{
    ...
    for (sl_decl_t* param = decl->dl_params; param;
            param = param->dl_list) {
        // check type is known
        if (!lookup_type_decl(sem_info, param->dl_type)) {
            elprintf("unknown type: '%T'\n", sem_info->si_filename,
                    param->dl_line, param->dl_type);
            result -= 1;
        }
        ...
        if (declare_in_current_scope(sem_info, param->dl_name, param->dl_type) < 0) {
            elprintf("second declaration of parameter '%s' in function '%s'\n",
                    sem_info->si_filename, param->dl_line,
                    param->dl_name, decl->dl_name);
            result -= 1;
        }
    }
    ...
    if (final_expr) {
        if (final_expr->ex_tag == SL_EXPR_RETURN) {
            ...
            elprintf("final expression in '%s' has type '%T' but expected '%T'",
                    sem_info->si_filename, final_expr->ex_line,
        ...
        if (ty_cmp(decl->dl_type, sem_info->si_builtin_types.void_type) != 0) {
            elprintf("no expressions in non-void function '%s'\n",
                    sem_info->si_filename, decl->dl_line);
            result -= 1;
        }
    }
...
static int verify_decl_struct(sem_info_t* sem_info, sl_decl_t* decl)
{
    ...
    for (sl_decl_t* field = decl->dl_params; field;
        ...
        if (!lookup_type_decl(sem_info, field->dl_type)) {
            elprintf("unknown type: '%T'\n", sem_info->si_filename,
                    field->dl_line, field->dl_type);
            result -= 1;
        }
        // TODO: check type is not recursive
...
int sem_verify_and_type_program(const char* filename, sl_decl_t* program)
{
    ...
    for (sl_decl_t* decl = program; decl; decl = decl->dl_list) {
        ...
        sl_type_t* type_of_decl = NULL;
        switch (decl->dl_tag) {
            case SL_DECL_STRUCT:
                type_of_decl = ty_type_name(decl->dl_name);
                break;
            case SL_DECL_FUNC:
                type_of_decl = ty_type_func();
                break;
            case SL_DECL_PARAM:
                assert(0 && "param at top-level ...");
                break;
        ...
        if (declare_in_current_scope(&sem_info, decl->dl_name, type_of_decl) < 0) {
            elprintf("second declaration of '%s' in module\n",
                    sem_info.si_filename, decl->dl_line, decl->dl_name);
            result -= 1;
        }
    }
